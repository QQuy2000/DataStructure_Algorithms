...
private:
    ...
    XNode* rotateRight(XNode* root){
        //YOUR CODE HERE
        XNode* newNode = root->pLeft;
        root->pLeft = newNode->pRight;
        newNode->pRight = root;
        root->updateHeight();
        newNode->updateHeight();
        return newNode;
    ...
    XNode* add(XNode* root, XNode* newNode){
        ...
        //check if tree is NULL or we find the position of newnode
        if(root == NULL) return newNode;

        //if not NULL, we find the position of newnode
        ...
        //after adding we must check if tree must be balanced 
    ...
    XNode* remove(XNode* root, K key, bool& success, V& retValue){
        ...
        if(key <  root->data.key){
            ...
            if((root->pLeft == NULL) || (root->pRight == NULL)){
                newRoot = ((root->pLeft == NULL) ? root->pRight : root->pLeft);
                success = true;
                ...
                while(largestOnLeft->pRight != NULL){
