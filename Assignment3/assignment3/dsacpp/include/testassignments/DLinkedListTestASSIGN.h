...
#ifndef DLINKEDLISTTESTASSIGN_H
#define DLINKEDLISTTESTASSIGN_H
...
TEST_CASE( "DLinkedList<int>: appending items = add(*)" ) {
...
TEST_CASE( "DLinkedList<int>: add first = add(0, *)" ) {
    DLinkedList<int> list;
    REQUIRE( list.size() == 0 );
    ...
    for(int idx=0; idx < nsizes; idx++){
        ...
        int i;
        for(it=list.bbegin(), i=0; it != list.bend(); it--, i++){
            REQUIRE(*it == items[i]);
        }
...
TEST_CASE( "DLinkedList<int>: add last = add(size, *)" ) {
    DLinkedList<int> list;
    REQUIRE( list.size() == 0 );
...
TEST_CASE( "DLinkedList<int>: add at k (random)= add(k, *)" ) {
    DLinkedList<int> list;
    REQUIRE( list.size() == 0 );
...
//Removal tests
TEST_CASE( "DLinkedList<int>: remove the first item = removeAt(0)" ) {
    DLinkedList<int> list;
    REQUIRE( list.size() == 0 );
...
TEST_CASE( "DLinkedList<int>: remove the last item = removeAt(size-1)" ) {
    DLinkedList<int> list;
    REQUIRE( list.size() == 0 );
...
TEST_CASE( "DLinkedList<int>: remove at k (random) = removeAt(k)" ) {
    DLinkedList<int> list;
    REQUIRE( list.size() == 0 );
...
TEST_CASE( "DLinkedList<int>: removeItem = removeItem(item)" ) {
    DLinkedList<int> list;
    REQUIRE( list.size() == 0 );
...
//test get
TEST_CASE( "DLinkedList<int>: get item at  = get(k)" ) {
    DLinkedList<int> list;
    REQUIRE( list.size() == 0 );
...
TEST_CASE( "DLinkedList<int>: indexOf(item)" ) {
    DLinkedList<int> list;
    REQUIRE( list.size() == 0 );
...
TEST_CASE( "DLinkedList<int>: contains" ) {
    DLinkedList<int> list;
    REQUIRE( list.size() == 0 );
...
TEST_CASE("DLinkedList<int>: iterator -> remove"){
    DLinkedList<int> list;
    REQUIRE( list.size() == 0 );
...
TEST_CASE("DLinkedList<int>: iterator backward"){
    DLinkedList<int> list;
    REQUIRE( list.size() == 0 );

    SUBCASE("backward iterator -> equality"){
        int* sizes = genIntArray(nsizes, 0, max_length, true, 2021);
        for(int idx=0; idx < nsizes; idx++){
            list.clear();
            REQUIRE( list.size() == 0 );

            //create list with size items:
            int size = sizes[idx];
            int* items = genIntArray(size, -500, 500, true, 2021);
            for(int i=0; i < size; i++) list.add(items[i]);
            ...
            DLinkedList<int>::BWDIterator it;
            int i;
            for(it = list.bbegin(), i=size-1; it != list.bend(); it--, i--){
                REQUIRE(*it == list.get(i));
            }
    ...
    SUBCASE("backward iterator -> remove"){
        int* sizes = genIntArray(nsizes, 0, max_length, true, 2021);
        for(int idx=0; idx < nsizes; idx++){
            list.clear();
            REQUIRE( list.size() == 0 );
